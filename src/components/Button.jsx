...
export default function Button({cards, onRemove, onToggle}) {
  ...
  return (
    <div className='flex flex-col md:grid md:grid-cols-3 gap-5 w-full justify-center items-center'>
     {cards.map((card,  i)=>(
      <div key={i} className='w-full dark:bg-[hsl(226,25%,17%)]   bg-white rounded-xl shadow-md p-4 pt-3 flex flex-col gap-12'>
      ...
      <button onClick={()=> onRemove(card.id)} className='focus:border-red-600 dark:focus:bg-hsl(225,23%,24%) dark:text-white focus:bg-[hsl(217,61%,90%)] flex justify-center items-center hover:text-white hover:bg-[hsl(3,77%,44%)] text-[hsl(227,75%,14%)] dark:border-white border-1 rounded-3xl  w-20'>Remove</button>
